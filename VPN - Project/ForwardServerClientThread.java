...
public class ForwardServerClientThread extends Thread
{
    ...
    public ForwardServerClientThread(Socket aClientSocket, String serverhost, int serverport)
    {
        ...
        this.SerSEncrypter = Handshake.getSessionEncrypter();
        this.SerCDecrypter = Handshake.getSessionDecrypter();
    }
        ...
        try {
           ...
           OutputStream serverOut = mServerSocket.getOutputStream();
            ...
            if(getListenSocket() == null){
                if(SerSEncrypter != null) {
                    serverOut = SerSEncrypter.openCipherOutputStream(serverOut);
                }
                if(SerCDecrypter != null) {
                    serverIn = SerCDecrypter.openCipherInputStream(serverIn);
                }
            } else {
                if(SerSEncrypter != null) {
                    clientOut = SerSEncrypter.openCipherOutputStream(clientOut);
                }
                if(SerCDecrypter != null) {
                    clientIn = SerCDecrypter.openCipherInputStream(clientIn);
                }
            }
